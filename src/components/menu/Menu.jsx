...
import { Search, X, ChevronDown, CornerDownRight } from "lucide-react";
import { useCustomNavigate } from "../../hooks/useNavigate";
import { menuItem } from "../../lib/data/menu-item";
    ...
    <div className="bg-[#1E2124] text-white h-screen p-3 overflow-y-auto">
      <div className="flex justify-between items-center w-[98%] mx-auto">
        ...
        <div className="hidden md:flex gap-1">
          ...
          <div className="flex">
            <Button size="icon" className="hover:bg-white hover:text-black">
              <Search strokeWidth={0.9} />
            </Button>

            <Button
              size="icon"
              className="bg-[#1E2124] text-white hover:bg-white hover:text-black"
              onClick={() => goTo("/")}
            >
              <X strokeWidth={0.9} />
            </Button>
      ...
      <div className="grid grid-cols-3 gap-12 w-[95%] mx-auto">
        <ul className="flex flex-col gap-3 mt-12">
          <p className="border-t uppercase text-sm pt-4">navigation</p>
          {menuItem.map((item) => (
            <li key={item.id} className="flex flex-col">
              {/* MAIN ITEM */}
              <span className="text-xl font-medium text-white hover:text-gray-500 cursor-pointer">
                {item.title}
              </span>

              {/* ALWAYS SHOW SUBMENU */}
              {item.submenu && (
                <ul className="ml-1 mt-2 flex flex-col gap-1 text-gray-400">
                  {item.submenu.map((sub) => (
                    <li
                      key={sub.id}
                      className="cursor-pointer hover:text-gray-300 flex gap-2 text-sm"
                    >
                      <CornerDownRight />
                      {sub.title}
                    </li>
